...
async function extractFramesFromGif(buffer: Buffer): Promise<FrameData[]> {
  // Write buffer to temp file (remove after)
  const tmpFile = `${import.meta.dir}/tmp_gif_${Date.now()}.gif`;
  await Bun.write(tmpFile, buffer);

  const frameData = await gifFrames({ url: tmpFile, frames: "all", outputType: "png" });

  Bun.file(tmpFile).unlink().catch(() => {});
